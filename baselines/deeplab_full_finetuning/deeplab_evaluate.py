"""DeepLabV3+ (ResNet50 Encoder) Full Fine-Tuning - Fold Evaluation"""
...
from evaluation.metrics import batch_metrics, nanstats
...
def evaluate_checkpoint(ckpt_path, image_files, mask_files, val_idx):
    ...
    summary = {k: nanstats(v) for k, v in all_metrics.items()}
    return summary
...
def main():
    ...
    for ckpt_path in ckpts:
        ...
        # NOTE: persist each fold's val indices during training (e.g. as .npy)
        # so evaluation reloads the exact same splits used at train time.
